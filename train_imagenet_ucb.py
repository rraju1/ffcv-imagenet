...
from ffcv.traversal_order import QuasiRandom, Random, Sequential
...
Section('training', 'training hyper param stuff').params(
    ...
    alpha=Param(float, 'exponential moving weight', default=0.8),
    confidence=Param(float, 'confidence to weight variance term', default=1.0),
)
...
class ImageNetTrainer:
    ...
    @param('training.distributed')
    @param('training.seed')
    def train(self, epochs, log_level, frac_percent, freq, distributed, seed):
        total_num_samples = 1281167
        # need to build the initial score dict
        counts_dict = dict.fromkeys(range(total_num_samples), 0)
        score_var = dict.fromkeys(range(total_num_samples), 0)
        ...
        for epoch in range(epochs):
            ...
            if epoch != 0 and epoch % freq == 0:
                score_dict, score_var = self.score_ema_var(score_dict, counts_dict, score_var)


        total = time.time() - start
        self.eval_and_log({'epoch':epoch, 'total time': total})
        with open(f'counts_{seed}_{frac_percent}.json', 'w+') as json_file:
            json.dump(counts_dict, json_file)
        if self.gpu == 0:
            ch.save(self.model.state_dict(), self.log_folder / 'final_weights.pt')
    
    def increment_scores(self, count_dict, samples):
        for idx in range(len(samples)):
            count_dict[samples[idx]] = count_dict[samples[idx]] + 1
    ...
    def score_ema_var(self, prev_scores_dict, count_dict, prev_var, frac_percent, distributed):
        ...
        threshold_scores = {key:val for key, val in ucb.items() if val > threshold}
        pool = np.array(list(threshold_scores.keys()))
        self.increment_scores(count_dict, pool)
        self.train_loader.indices = pool
        self.train_loader.traversal_order = Random(self.train_loader) if distributed else QuasiRandom(self.train_loader)
        return score_dict, scores_var
 
    @param('training.alpha')
    @param('training.confidence')
    def calc_var_scores(self, curr_scores, prev_scores, prev_var, alpha, confidence):
        total_num_samples = 1281167
        curr_var = dict.fromkeys(range(total_num_samples), 0)
        ucb = dict.fromkeys(range(total_num_samples), 0)
        for key in curr_scores:
            delta = curr_scores[key] - prev_scores[key] # delta_i = x_i - ema_i-1
            curr_scores[key] = prev_scores[key] + alpha * delta # ema_i = ema_i-1 + alpha * delta_i
            curr_var[key] = (1 - alpha) * (prev_var[key] + alpha * delta**2) # emvar_i = (1 - alpha) * (emavar_i-1 + alpha * delta_i^2)
            ucb[key] = curr_scores[key] + confidence*curr_var[key] # get the upper bound confidence of samples and scale it by conf
        return curr_scores, curr_var, ucb
