...
class ImageNetTrainer:
    ...
    def train(self, epochs, log_level, seed):
        ...
        forget_count = dict.fromkeys(range(total_num_samples), 0)
        prev_acc = dict.fromkeys(range(total_num_samples), 0)

        start = time.time()
        for epoch in range(epochs):
            res = self.get_resolution(epoch)
            self.decoder.output_size = (res, res)
            train_loss = self.train_loop(epoch, forget_count, prev_acc)
        ...
        with open(f'./forgot_{seed}.json', 'w+') as file:
            json.dump(forget_count, file)
    ...
    @param('logging.log_level')
    def train_loop(self, epoch, forget_scores, prev_acc, log_level):
        model = self.model
        model.train()
        losses = []
        ...
        for ix, (images, target, idx) in enumerate(iterator):
            ...
            current_acc = preds.eq(target.data.view_as(preds)).cpu().numpy()
            self.update_forget_stats(idx, current_acc, forget_scores, prev_acc)
    ...
    def update_forget_stats(self, indices, current_acc, forget_scores, prev_scores):
        i = 0
        for idx in indices:
            idx = idx.item()
            if prev_scores[idx] > current_acc[i]:
                forget_scores[idx] = forget_scores[idx] + 1
            prev_scores[idx] = current_acc[i]
            i = i + 1
